	    ...
	    mysqli_query($conexion,'SET NAMES "'. CHARSET .'"');//es importante esta linea para que los caracteres especiales funcione, tanto graficamente como logicamente
				...
				if($Incremento >'00:00:00'){ //Si el participante esta bloqueado
				   ...
				   $horaPHPlocal = date("Y-m-d  H:i:s", time()  + $salto_horario_PHPLocal);
					...
					$Insertar_4="UPDATE respuestas SET Hora_Respuesta= '$horaPHPlocal' WHERE ID_Participante='$Participante' AND Libro='$Libro' AND ID_Pregunta='13'";
						mysqli_query($conexion,$Insertar_4);

					//consulta para verificar si existe una respuesta correcta en la pregunta Nº 13
			    	$Consulta= "SELECT * FROM respuestas WHERE ID_Pregunta='13' AND ID_Participante= '$Participante' AND Libro='$Libro'";
			    	$Recordset= mysqli_query($conexion,$Consulta);
			    		...
			    		echo "<h3>Su repuesta es correcta y la encuentra en Éxodo 7:20. Pero no sumara puntos porque ya respondio esta pregunta</h3>";
						...
						$Actualizar="UPDATE respuestas SET Correcto= 1 WHERE ID_Participante='$Participante' AND Libro='$Libro' AND ID_Pregunta='13'";
						  mysqli_query($conexion,$Actualizar);

						//se verifica si el tiempo de respuesta esta vencido
						$Consulta_3="SELECT HoraMaximo FROM respuestas WHERE ID_Participante='$Participante' AND Libro='$Libro' AND ID_Pregunta='13'";
						...
						$Consulta_4="SELECT Hora_Respuesta FROM respuestas WHERE ID_Participante='$Participante' AND Libro='$Libro' AND ID_Pregunta='13'";
						$Recordset_4= mysqli_query($conexion,$Consulta_4);
	   		 			...
	   		 			if($HM < $Verificar["Hora_Respuesta"]){
			 				echo "<h2>Correcto, sin embargo no sumará puntos por tiempo vencido </h2> <p>Éxodo 7:20.</p>";
						}
						else{
							echo "<h2>Correcto. Felicitaciones</h2> <p>Éxodo 7:20.</p>";
