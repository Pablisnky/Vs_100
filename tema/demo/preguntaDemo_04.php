...
session_start();//se inicia sesion para llamar las variables $_SESSION creadas en otros archivos, o para crear una nueva.
	 ...
	 if(!isset($_SESSION["ID_PD"])){//sino hay nada almacenado en la variable superglobal devuelve a la pagina de inicio
		...
		define("PREGUNTA_ACTUAL", 4); // definiendo una constante para identificar el número de la pregunta actual
		define("PREGUNTA_ANTERIOR", 3); // definiendo una constante para identificar el número de la pregunta anterior
...
<html lang="es">
	...
	<?php
		...
		if($Respondida>2){//Condicion que impide entrar a una pregunta sino a respondido la pregunta previa, $_SESSION creada en sumaPuntaje.php
	?>
	...
	<body onload="llamar_puntaje_Demo()"><!--funcion Ajax en puntaje.js que accede a BD para sumar el puntaje del participante -->
	    ...
	    <div class="Secundario">
			...
			<div>
				<p class="pregunta">El punto mas alto en Ameríca es:</p>
			</div>
			<div class="Quinto">
				<div class="Quinto_2">
					<p id="respuesta_a" class="efecto" onclick="sonidoInCorrecto(); pauseAudio(); llamar_bloqueo_Demo()">El Everest.</p>
					<p id="respuesta_b" class="efecto" onclick="sonidoInCorrecto(); pauseAudio(); llamar_bloqueo_Demo()">El Monte sinai.</p>
				</div>
				<div class="Quinto_2">
					<p id="respuesta_c" class="efecto" onclick="sonidoCorrecto(); llamar_sombrear_Demo(); pauseAudio(); setTimeout(llamar_puntaje_Demo,200);">El Aconcagua.</p>
					<p id="respuesta_d" class="efecto" onclick="sonidoInCorrecto(); pauseAudio(); llamar_bloqueo_Demo()">El Pico Bolivar.</p>
				</div>
			</div>
			...
			<div class="contenedor_6" id="Flecha">
				<a  href='preguntaDemo_05.php'><span class="icon-arrow-right parpadea" title="Siguiente"></span></a>					
			</div>
		</div>
	</body>
</html>
