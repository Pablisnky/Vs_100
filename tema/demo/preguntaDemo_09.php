...
session_start();//se inicia sesion para llamar las variables $_SESSION creadas en otros archivos, o para crear una nueva.
	 ...
	 if(!isset($_SESSION["ID_PD"])){//sino hay nada almacenado en la variable superglobal devuelve a la pagina de inicio
		...
		define("PREGUNTA_ACTUAL", 9); // definiendo una constante para identificar el número de la pregunta actual
		define("PREGUNTA_ANTERIOR", 8); // definiendo una constante para identificar el número de la pregunta anterior
...
<html lang="es">
	...
	<?php
		...
		if($Respondida>7){//Condicion que impide entrar a una pregunta sino a respondido la pregunta previa, $_SESSION creada en sumaPuntaje.php
	?>
	...
	<body onload="llamar_puntaje_Demo()"><!--funcion Ajax en puntaje.js que accede a BD para sumar el puntaje del participante -->
	    ...
	    <div class="Secundario">
			...
			<div class="Quinto">
				<div class="Quinto_2">
					<p id="respuesta_a" class="efecto" onclick="llamar_bloqueo_Demo();">La cascada del Vino.</p>
					<p id="respuesta_b" class="efecto" onclick="llamar_bloqueo_Demo()">El Orinoco.</p>
				</div>
				<div class="Quinto_2">
					<p id="respuesta_c" class="efecto" onclick="llamar_bloqueo_Demo()">El mirador caraqueño.</p>
					<p id="respuesta_d" class="efecto" onclick="llamar_sombrear_Demo(); setTimeout(llamar_puntaje_Demo,200)">El Salto Angel.</p>
				</div>
			</div>
			...
			<div class="contenedor_6" id="Flecha">
				<a  href='preguntaDemo_10.php'><span class="icon-arrow-right parpadea" title="Siguiente"></span></a>					
			</div>
		</div>
	</body>
</html>
