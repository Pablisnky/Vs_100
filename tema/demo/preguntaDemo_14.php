...
session_start();//se inicia sesion para llamar las variables $_SESSION creadas en otros archivos, o para crear una nueva.
	 ...
	 if(!isset($_SESSION["ID_PD"])){//sino hay nada almacenado en la variable superglobal devuelve a la pagina de inicio
		...
		define("PREGUNTA_ACTUAL", 14); // definiendo una constante para identificar el número de la pregunta actual
...
<html lang="es">
	<head>
		<title>Pregunta Nº <?php echo $Num_Pregunta;?></title>
	...
	<body onload="llamar_puntaje_Demo()"><!--funcion Ajax en puntaje.js que accede a BD para sumar el puntaje del participante -->
	    ...
	    <div class="Secundario">
			<div class="encabezado">
				<h1 class="anula h1_2">Sabado Joven</h1>
	    	</div>
			...
			<div>
				<p class="pregunta">El día que el rey Saúl murió, también murieron sus hijos, su escudero  y todos sus guerreros. Los de Jabes de Galaad, quemaron su cuerpos y enterraron sus cenizas bajo un árbol, luego ayunaron. Por cuantos días? </p>
			</div>
			<div class="Quinto">
				<div class="Quinto_2">
					<p id="respuesta_d" class="efecto" onclick="sonidoCorrecto(); llamar_sombrear_Demo(); pauseAudio(); setTimeout(llamar_puntaje_Demo,200)">siete</p>
					<p id="respuesta_a" class="efecto" onclick="sonidoInCorrecto();  pauseAudio();llamar_bloqueo_Demo();">cinco </p>
				</div>
				<div class="Quinto_2">
					<p id="respuesta_c" class="efecto" onclick="sonidoInCorrecto(); pauseAudio();llamar_bloqueo_Demo()">diez</p>
					<p id="respuesta_b" class="efecto" onclick="sonidoInCorrecto(); pauseAudio();llamar_bloqueo_Demo()">tres</p>
				</div>
			</div>
			...
			<div class="contenedor_6" id="Flecha">
				<a  href='preguntaDemo_15.php'><span class="icon-arrow-right parpadea" title="Siguiente"></span></a>					
			</div>
		</div>
	</body>
</html>
