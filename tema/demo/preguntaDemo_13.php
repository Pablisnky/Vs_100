...
session_start();//se inicia sesion para llamar las variables $_SESSION creadas en otros archivos, o para crear una nueva.
	 ...
	 if(!isset($_SESSION["ID_PD"])){//sino hay nada almacenado en la variable superglobal devuelve a la pagina de inicio
		...
		define("PREGUNTA_ACTUAL", 13); // definiendo una constante para identificar el número de la pregunta actual
...
<html lang="es">
	...
	<body onload="llamar_puntaje_Demo()"><!--funcion Ajax en puntaje.js que accede a BD para sumar el puntaje del participante -->
	    ...
	    <div class="Secundario">
			...
			<div>
				<p class="pregunta">Cuando los filisteos tomaron el arca, el nieto de Elí nació ese mismo día, pues la esposa de Finees, hijo de Eli, al conocer que éste había muerto, le sobrevino el parto, y llamó a su hijo: </p>
			</div>
			<div class="Quinto">
				<div class="Quinto_2">
					<p id="respuesta_a" class="efecto" onclick="sonidoInCorrecto();  pauseAudio();llamar_bloqueo_Demo();">Samuel</p>
					<p id="respuesta_d" class="efecto" onclick="sonidoCorrecto(); llamar_sombrear_Demo(); pauseAudio(); setTimeout(llamar_puntaje_Demo,200)">Icabod</p>
				</div>
				<div class="Quinto_2">
					<p id="respuesta_b" class="efecto" onclick="sonidoInCorrecto(); pauseAudio();llamar_bloqueo_Demo()">Absalón</p>
					<p id="respuesta_c" class="efecto" onclick="sonidoInCorrecto(); pauseAudio();llamar_bloqueo_Demo()">Ahimelec </p>
				</div>
			</div>
			...
			<div class="contenedor_6" id="Flecha">
				<a  href='preguntaDemo_14.php'><span class="icon-arrow-right parpadea" title="Siguiente"></span></a>					
			</div>
		</div>
	</body>
</html>
