<?php
	$Participante = $_GET["val_1"];//se recibe el ID_Participante desde preguntaGenesis_05.php
	$Pais = $_GET["val_2"];//se recibe el nombre del Pais desde preguntaGenesis_05.php
	...
	include("../../conexion/Conexion_BD.php");
				...
				if($Incremento >'00:00:00'){ //Si el participante esta bloqueado
					...
					//se configura el sistema para que trabaje con la hora local.
					...
					$Insertar_4="UPDATE respuestas SET Hora_Respuesta= '$horaPHP' WHERE ID_Participante='$Participante' AND Pais='$Pais' AND ID_Pregunta='5'";
							mysqli_query($conexion,$Insertar_4);

					//consulta para verificar si existe una respuesta correcta en la pregunta Nº 5
			    	$Consulta= "SELECT * FROM respuestas WHERE ID_Pregunta='5' AND ID_Participante= '$Participante' AND Pais='$Pais'";
			    	...
			    	if(($Verificar["Correcto"])==1){ //si existe una respuesta correcta en la base de datos.
						...
						$Actualizar="UPDATE respuestas SET Correcto= 1 WHERE ID_Participante='$Participante' AND Pais='$Pais' AND ID_Pregunta='5'";
						  mysqli_query($conexion,$Actualizar);

						//se verifica si el tiempo de respuesta esta vencido
						$Consulta_3="SELECT HoraMaximo FROM respuestas WHERE ID_Participante='$Participante' AND Pais='$Pais' AND ID_Pregunta='5'";
						...
						$Consulta_4="SELECT Hora_Respuesta FROM respuestas WHERE ID_Participante='$Participante' AND Pais='$Pais' AND ID_Pregunta='5'";
