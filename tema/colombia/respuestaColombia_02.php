<?php
	$Participante = $_GET["val_1"];//se recibe el ID_Participante desde preguntaColombia_01.php
	$Pais = $_GET["val_2"];//se recibe el nombre del pais desde preguntaColombia_01.php
	...
	include("../../conexion/Conexion_BD.php");
				...
				if($Incremento >'00:00:00'){ //Si el participante esta bloqueado
				   ...
				   // $salto_horario_PHPLocal = -0.5 * 60 * 60;//se restan 30 minutas, porque el servidor PHPLocal esta adelantado
					...
					$Insertar_4="UPDATE respuestas SET Hora_Respuesta= '$horaPHP' WHERE ID_Participante='$Participante' AND Pais='$Pais' AND ID_Pregunta='2'";
							mysqli_query($conexion,$Insertar_4);

					//consulta para verificar si existe una respuesta correcta en la pregunta Nº 2
			    	$Consulta= "SELECT * FROM respuestas WHERE ID_Pregunta='2' AND ID_Participante= '$Participante' AND Pais='$Pais'";
			    	...
			    	if(($Verificar["Correcto"])==1){ //si existe una respuesta correcta en la base de datos.
			    		echo "<h3>Su repuesta es correcta pero no sumará puntos porque ya respondio esta pregunta</h3><p>'Pendiente por Bibliografia'</p>";
						...
						$Actualizar="UPDATE respuestas SET Correcto= 1 WHERE ID_Participante='$Participante' AND Pais='$Pais' AND ID_Pregunta='2'";
						  mysqli_query($conexion,$Actualizar);

						//se verifica si el tiempo de respuesta esta vencido
						$Consulta_3="SELECT HoraMaximo FROM respuestas WHERE ID_Participante='$Participante' AND Pais='$Pais' AND ID_Pregunta='2'";
						...
						$Consulta_4="SELECT Hora_Respuesta FROM respuestas WHERE ID_Participante='$Participante' AND Pais='$Pais' AND ID_Pregunta='2'";
						$Recordset_4= mysqli_query($conexion,$Consulta_4);
	   		 			...
	   		 			if($HM < $Verificar["Hora_Respuesta"]){
			 				echo "<h3>Correcto, sin embargo no sumará puntos por tiempo vencido </h3> <p>'Pendiente por Bibliografia'</p>";
						}
						else{
