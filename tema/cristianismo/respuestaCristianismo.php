...
// ---------------------------------------------------------------------------------------------------
	...
	if(($Verificar["Correcto"]) == 1){ 
		...
		$Actualizar_4="UPDATE respuestas SET Hora_Respuesta= NOW() WHERE ID_Participante='$Participante' AND Tema='$Tema' AND ID_Pregunta='$Pregunta' AND ID_PP = '$CodigoPrueba' ORDER BY ID_Respuesta DESC LIMIT 1";
		...
		$Recordset_4= mysqli_query($conexion,$Consulta_4);
	   	...
	   	if($HM < $Verificar["Hora_Respuesta"]){
			...
			$Actualizar_4= "UPDATE respuestas SET correcto = 1, Hora_Respuesta = NOW() WHERE ID_Participante='$Participante' AND ID_Pregunta='$Pregunta' AND Tema ='$Tema' AND ID_PP = '$CodigoPrueba'";
			mysqli_query($conexion,$Actualizar_4);
		}
		else{
			echo "<h3>Correcto. Felicitaciones</h3>";
			//se actualiza en la BD la hora a la que respondio la pregunta
			$Actualizar_5= "UPDATE respuestas SET correcto = 1, Hora_Respuesta = NOW() WHERE ID_Participante='$Participante' AND ID_Pregunta='$Pregunta' AND Tema ='$Tema' AND ID_PP = '$CodigoPrueba'";
		...
		$insertar= "INSERT INTO respuestas(ID_Pregunta, ID_Participante, ID_PP, Tema, Correcto, Hora_Pregunta, Hora_Respuesta) VALUES('$Pregunta', '$Participante', '$CodigoPrueba', '$Tema', '$Hora_Pregunta', 1, NOW())";
