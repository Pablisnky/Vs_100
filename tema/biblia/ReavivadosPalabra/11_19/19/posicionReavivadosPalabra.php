...
// session_start();//se inicia sesion para llamar a las variables $_SESSION creadas en otros archivos, o para crearlas si es necesario
	...
	$ID_PP = $_GET["ID_PP"];
	// echo "Numero de pregunta= " . $Pregunta . "<br>";
	...
	// echo "Puntos: " . $Puntaje . "<br>";
	
	$_SESSION["Puntos"] = $Puntaje; //Se crea la sesion que indica en que pregunta va el participante, esta sesion se invoca nuevamente en respuesta.php
	...
	if($Puntaje==0){
		...
		//Se inserta la pregunta que le toco de las 12 formuladas.
		$Puntuar= "UPDATE respuestas SET Num_Pregunta_Alea= $Aleatorio_1 WHERE ID_Participante='$participante' AND ID_PP = '$ID_PP' AND ID_Pregunta= '$Pregunta'";
		mysqli_query($conexion, $Puntuar);
	}
	else if ($Puntaje==1){
		include("preguntaBiblia_ReavivadosPalabra_$Aleatorio_2.php");
		//Se inserta la pregunta que le toco de las 12 formuladas.
		$Puntuar_1= "UPDATE respuestas SET Num_Pregunta_Alea= $Aleatorio_2 WHERE ID_Participante='$participante' AND ID_PP = '$ID_PP' AND ID_Pregunta= '$Pregunta'";
		mysqli_query($conexion, $Puntuar_1);
	}
	else if ($Puntaje==2){
		include("preguntaBiblia_ReavivadosPalabra_$Aleatorio_3.php");
		//Se inserta la pregunta que le toco de las 12 formuladas.
		$Puntuar_2= "UPDATE respuestas SET Num_Pregunta_Alea= $Aleatorio_3 WHERE ID_Participante='$participante' AND ID_PP = '$ID_PP' AND ID_Pregunta= '$Pregunta'";
		mysqli_query($conexion, $Puntuar_2);
	}
		else if ($Puntaje==3){
		include("preguntaBiblia_ReavivadosPalabra_$Aleatorio_4.php");
		//Se inserta la pregunta que le toco de las 12 formuladas.
		$Puntuar_3= "UPDATE respuestas SET Num_Pregunta_Alea= $Aleatorio_4 WHERE ID_Participante='$participante' AND ID_PP = '$ID_PP' AND ID_Pregunta= '$Pregunta'";
		mysqli_query($conexion, $Puntuar_3);
	}
	else if ($Puntaje==4){
		include("preguntaBiblia_ReavivadosPalabra_$Aleatorio_5.php");
		//Se inserta la pregunta que le toco de las 12 formuladas.
		$Puntuar_4= "UPDATE respuestas SET Num_Pregunta_Alea= $Aleatorio_5 WHERE ID_Participante='$participante' AND ID_PP = '$ID_PP' AND ID_Pregunta= '$Pregunta'";
