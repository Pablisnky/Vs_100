<?php   
    ...
    $Consulta="SELECT * FROM respuestas_demo WHERE ID_Participante='$Participante' AND ID_Pregunta='$Pregunta' AND Tema ='$Tema' AND ID_PP = '$CodigoPrueba'";
   ...
   //$salto_horario_PHPLocal = -0.5 * 60 * 60;//se restan 30 minutas, porque el servidor PHP esta adelantado
    ...
    if($VerificarPregunta == 0){ 
       ...
       $Consulta_2= "SELECT DATE_ADD(NOW(),INTERVAL 2 minute) AS minutos";//este codigo  funciona para añadir tiempo , (6 hour - 3 minute)(en local 2 minutos, en remoto 122 minutos)
        ...
        $insertar= "INSERT INTO respuestas_demo(ID_Pregunta, ID_Participante, ID_PP, Tema, Hora_Pregunta, HoraMaximo) VALUES('$Pregunta', '$Participante', '$CodigoPrueba', '$Tema', NOW(), '$Incremento_2')";
...
//----------------------------------------------------------------------------------------------------------------
  ...
  $Consulta_0= "SELECT DATE_FORMAT(HoraMaximo, '%Y/%m/%d') FROM respuestas_demo WHERE ID_Participante= '$Participante' AND ID_Pregunta='$Pregunta' AND Tema ='$Tema' AND ID_PP = '$CodigoPrueba'";
  ...
  $Consulta_1= "SELECT DATE_FORMAT(HoraMaximo, '%H:%i:%s') FROM respuestas_demo WHERE ID_Participante= '$Participante'AND ID_Pregunta='$Pregunta' AND Tema ='$Tema' AND ID_PP = '$CodigoPrueba'";
