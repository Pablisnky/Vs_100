...
// session_start();    //se inicia sesion para llamar a una variable 
    ...
    $ID_ParticipanteTrivia= $_SESSION["ID_ParticipanteTrivia"];//en esta sesion se guarda el id del participante, sesion creada en respuestaTrivia.php
...
<html lang="es">
	...
	<body>
		<?php
	    	...
	    	$Consulta="SELECT * FROM participante_trivia WHERE ID_ParticipanteTrivia='$ID_ParticipanteTrivia'";//se plantea la consulta
			$Recordset = mysqli_query($conexion, $Consulta);//se manda a ejecutar la consulta
			$ParticipanteTrivia= mysqli_fetch_array($Recordset);
		?>
		
    	<input type="text" class="ocultar" id="ID_ParticipanteTrivia" value="<?php echo $ID_ParticipanteTrivia;?>"><!-- se utiliza para enviar a puntaje.js-->
		<?php
				//Se consulta si el participante a respondido la pregunta anterior
				$Consulta="SELECT * FROM respuestas_trivias WHERE ID_ParticipanteTrivia='$ID_ParticipanteTrivia' AND Correcto='1'";
				...
				if($Respondida>4){//Condicion que impide entrar a una pregunta sino a respondido la pregunta previa, $_SESSION creada en sumaPuntaje.php

		    	//se actualiza en la BD que no tiene esta prueba pendiente
				$Actualiza="UPDATE participante_trivia SET Prueba_Activa= 0 WHERE ID_ParticipanteTrivia='$ID_ParticipanteTrivia'";
			...
			<div class="ultimaPregunta">
				<?php
			    	...
			    	$Consulta_1="SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(TIMEDIFF(Hora_Respuesta,Hora_Pregunta)))) AS TiempoTotal FROM respuestas_trivias WHERE ID_ParticipanteTrivia='$ID_ParticipanteTrivia' ";
            	...
            	<?php
            		...
            		$Consulta_3="SELECT COUNT(*) AS Pus FROM participante_trivia WHERE puntos >= '$Puesto'  ";
			    	...
			    	$Consulta_4="SELECT * FROM participante_trivia";//se plantea la consulta
		    	...
		    	<div class="tabla_3">
	                <table>
						...
						$Consulta_2="SELECT ID_Pregunta, punto_ganado, SEC_TO_TIME(SUM(TIME_TO_SEC(TIMEDIFF(Hora_Respuesta,Hora_Pregunta)))) AS tiempo FROM respuestas_trivias WHERE ID_ParticipanteTrivia='$ID_ParticipanteTrivia' AND Correcto = 1 AND punto_ganado > 0 GROUP BY ID_Pregunta ";
	                    ...
	                    <tbody>
	                        <tr>
	                            <td class="tabla_0"><?php echo $TiempoPregunta["ID_Pregunta"];?></td>
	                           	<td class="tabla_1"><?php echo $TiempoPregunta["punto_ganado"];?></td> 
	                ...
	                <table>
	                 ...
	                 // puntos descontados por cada pregunta incorre
						$Consulta_3="SELECT ID_Pregunta, SEC_TO_TIME(SUM(TIME_TO_SEC(TIMEDIFF(Hora_Respuesta,Hora_Pregunta)))) AS tiempo, SUM(punto_ganado) As penalizacion FROM respuestas_trivias WHERE ID_ParticipanteTrivia='$ID_ParticipanteTrivia' AND punto_ganado <= 0.000 GROUP BY ID_Pregunta ";
						$Recordset_3 = mysqli_query($conexion, $Consulta_3);//se manda a ejecutar la consulta
